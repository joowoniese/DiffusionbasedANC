...
class ResidualBlock(nn.Module):
  def __init__(self, n_mels, residual_channels, dilation, uncond=False):
    ...
    # 🔹 기존 spectrogram 대신 target을 조건으로 사용
    if not uncond:
  ...
  def forward(self, x, diffusion_step, conditioner=None):
    ...
    if self.conditioner_projection is None:
      ...
      # 🔹 기존 spectrogram 대신 target을 condition으로 사용
      conditioner = self.conditioner_projection(conditioner)

      # Conditioner 크기를 y와 맞추기 위해 업샘플링
...
class DiffWave(nn.Module):
  ...
  def forward(self, audio, diffusion_step, target=None):
    # 🔹 spectrogram을 target으로 변경
    assert (target is None and self.target_upsampler is None) or \
           (target is not None and self.target_upsampler is not None)
