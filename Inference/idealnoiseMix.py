...
output_dir = "/hdd_ext/hdd3/joowoniese/diffwave4/testset/ideal_output_mixed/"

os.makedirs(output_dir, exist_ok=True)

# antinoise에 있는 모든 _anti.wav 파일
antinoise_files = [f for f in os.listdir(antinoise_dir) if f.endswith("_anti.wav")]

# music 디렉토리의 모든 wav 파일
...
for music_file in music_files:
    ...
    for anti_file in antinoise_files:
        noise_path = os.path.join(antinoise_dir, anti_file)
        anti_base = anti_file.replace("_anti.wav", "")
        output_filename = f"{song_base}_{anti_base}_mixed.wav"
        output_path = os.path.join(output_dir, output_filename)

        print(f"[MIXING] {music_file} + {anti_file}")
