...
def train(args, params):
    dataset = from_path(args.input_dirs, args.target_dirs, params)

    # 🚨 데이터셋 크기 확인
    print(f"[DEBUG] Dataset size: {len(dataset)}")
    if len(dataset) == 0:
        raise ValueError("[ERROR] Dataset is empty! Check input_dirs and target_dirs.")
...
def train_distributed(replica_id, replica_count, port, args, params):
    ...
    dataset = from_path(args.input_dirs, args.target_dirs, params, is_distributed=True)  # ✅ 수정 완료
