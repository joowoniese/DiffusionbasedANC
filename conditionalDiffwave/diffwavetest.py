...
# 멜 스펙트로그램 transform 생성 (power spectrogram만 생성)
...
for base in sorted(common_bases):
    ...
    # torchaudio.load: (waveform, sample_rate) 반환
    ...
    # 길이를 params.audio_len (여기서는 110250)로 맞추기
    ...
    # features 딕셔너리 생성 (평가는 forward만 수행)
    features = {"audio": input_waveform, "target": target_waveform}

    with torch.no_grad():
        # 모델 예측 (조건자로 target 사용)
        t = torch.randint(0, len(params.noise_schedule), [input_waveform.shape[0]], device=input_waveform.device)
        conditioner = target_waveform.unsqueeze(1)  # (B, 1, T)
        predicted_waveform = model(input_waveform, t, conditioner)
        end_time = time.time()  # ⏱ 종료 시간
        elapsed = end_time - start_time
        processing_times.append(elapsed)
        print(f"[{base}] 처리 시간: {elapsed:.3f}초")  # ▶️ 파일별 처리 시간 출력
        ...
        # custom loss 계산: 멜 스펙트로그램은 power 스펙트로그램 값을 그대로 사용합니다.
        ...
        total_loss = original_loss.item() + custom_loss.item()  # ← Total loss 계산
