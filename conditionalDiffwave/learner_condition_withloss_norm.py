        ...
        try:
            if self.summary_writer is None:
                self.summary_writer = SummaryWriter(
                    "/hdd_ext/hdd3/joowoniese/diffwave4/ConditionalDiffwavewithLoss2/event_logs_norm2/", purge_step=step)
            ...
            loss_log_file = os.path.join(
                "/hdd_ext/hdd3/joowoniese/diffwave4/ConditionalDiffwavewithLoss2/event_logs_norm2/",
                'losslog.csv')
            # 파일이 없을 경우에만 헤더 작성
            if not os.path.exists(loss_log_file):
                with open(loss_log_file, 'w', newline='') as file:
                    csv_writer = csv.writer(file)
                    csv_writer.writerow(['step', 'loss', 'original_loss', 'custom_loss', 'weight_original',
                                         'weight_custom'])
            ...
            with open(loss_log_file, 'a', newline='') as file:
                csv_writer = csv.writer(file)
                csv_writer.writerow(
                    [step, loss, original_loss, custom_loss, weight_original, weight_custom])
